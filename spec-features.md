...
* 自チームの設定検討の参考として、他の参加チームの WiFi 構成・機材情報を参照できること
...
* **チーム編集者**: 自チームの WiFi 構成登録、不具合報告、他チームを含む会場状況の確認を行う
...
他チームの WiFi 構成・機材情報は大会内の参加者および運営者向けの参考情報とし、未認証の一般閲覧範囲には含めない。
...
帯域ごとのチャンネル占有状況を可視化する。自チーム、他の参加チーム、野良 WiFi を同じ帯域ビューに並べ、設定検討時に参考にできるようにする。
...
* 自チームは強調色で表示する
* 他の参加チームは自チームと異なる通常色で表示する
* 野良 WiFi はグレー系で表示する
* バー、ツールチップ、詳細表示には表示元区分ラベルを表示する
* 不具合報告が閾値を超える場合は警告表示を行う
* 参加チーム由来のバーと野良 WiFi 由来のバーは、色とラベルで確実に見分けられるようにする
...
* 自チームの WiFi 構成と機材情報は、チーム編集者または運営者が編集できる
* 他の参加チームの WiFi 構成と機材情報は閲覧専用とし、編集操作は表示しない
* 他の参加チームの詳細情報は、チーム編集者または運営者のみ閲覧できる
* 野良 WiFi は運営者のみが登録・編集・削除できる
* 他チームの代表メールアドレスなど、WiFi 運用判断に不要な連絡先情報はチャンネルマップ詳細に表示しない
...
* 利用中 WiFi 構成の概要
* 使用機材のメーカー、型番、種別、対応帯域
* 既知の機材注意点

### 表示ルール

* 他チーム参照時は WiFi 運用判断に必要な情報のみを表示する
* 他チームの公開 WiFi 構成・機材情報は、チーム編集者または運営者のみ閲覧できる
* 代表メールアドレスなどの連絡先情報は、自チームまたは運営者以外には表示しない
...
自チームの基本情報と WiFi 構成を確認・編集する。他チームを開いた場合は、公開 WiFi 構成と公開機材情報を参考情報として閲覧する。
...
* 自チームまたは運営者の場合のみ、編集操作を表示する
* 他チーム閲覧時は編集操作を表示せず、WiFi 構成と機材情報を閲覧専用にする
* 他チーム閲覧時は代表メールアドレスなどの連絡先情報を表示しない
...
* 参加チーム由来の WiFi 構成と運営観測の野良 WiFi は、チャンネルマップ上では統合表示するが、データ上は区別して保持する
...
* 参加チーム由来の WiFi 情報としてチャンネルマップに表示する
* 自チーム以外の利用者には、公開 WiFi 構成として閲覧専用で表示する
...
* 参加チームが登録した WifiConfig とは別の情報として保持する
* チャンネルマップでは WifiConfig と同じ帯域ビューに表示するが、表示元区分を野良 WiFi として明示する
...
チャンネルマップ表示用に、参加チームの WifiConfig と運営観測の ObservedWifi を統合した読み取り専用データを表す。

### 属性

* id: string
* sourceType: "own_team" | "participant_team" | "observed_wifi"
* sourceId: string
* label: string
* teamId?: string
* teamName?: string
...
* 永続エンティティではなく、API レスポンスまたは画面表示用の派生データとして扱う
* sourceType により、自チーム、他の参加チーム、野良 WiFi を UI 上で区別する
* 他チームの代表メールアドレスなど、公開対象外の情報は含めない
...
* 報告時の自動補完に必要な情報が揃う
* チーム構成が会場全体の図に反映される
* 自チームの構成が、他の参加チームや野良 WiFi と区別できる状態でチャンネルマップに反映される
...
* 参加チームの WiFi 構成と野良 WiFi を同じチャンネルマップ上で比較できる

---

## 4.8 他チーム WiFi 情報参照フロー

### 手順

1. チーム編集者がチャンネルマップまたはチーム一覧を開く
2. 表示元区分で参加チームのみ、野良 WiFi のみ、または全体表示を切り替える
3. 他チームの WiFi 構成バーまたはチーム詳細を開く
4. 公開 WiFi 構成、機材型番、既知の注意点を確認する
5. 必要に応じて自チームの WiFi 構成見直しへ移動する

### 期待結果

* 自チームの設定検討時に、他チームの利用チャンネルや機材情報を参考にできる
* 他チーム情報は閲覧専用で表示され、誤って編集できない
* 参加チーム由来の情報と野良 WiFi 由来の情報を混同しない
...
* 失効・再発行が可能であること
* 他チーム閲覧時は、代表メールアドレスなど WiFi 運用判断に不要な連絡先情報を公開しないこと
...
* 機材仕様登録
* WiFi 構成登録（最大 3 件）
* 帯域別チャンネルマップ表示（自チーム / 参加チーム / 野良 WiFi の区別表示を含む）
* 他チームの公開 WiFi 構成・機材情報の閲覧
* 野良 WiFi の手入力・CSV 取込
* 不具合の簡易報告
* オフライン保存と後同期
* ベストプラクティス表示
