...
* トークン照合後、長期トークン本体を短期 Cookie または署名済みセッションへ交換し、以後の API 認可には短期セッションを使う
* 短期セッションには `teamId`、`tournamentId`、`role`、失効確認に必要な `teamAccessId` または session id を含める
* 長期トークン本体は Cookie に保存しない
...
* 短期セッション有効期限: 1 日から 7 日程度
* 再発行時の DB 更新は transaction で行い、旧トークン失効と新トークン保存の片方だけが成功しないようにする
...
* 自チーム、他の参加チーム、野良 WiFi を同じ帯域ビューに混在表示し、表示元区分ごとの色・ラベル・凡例を制御しやすい
...
* `WifiConfig` は参加チーム由来の WiFi 情報、`ObservedWifi` は野良 WiFi または運営観測 WiFi として扱う
* API または frontend adapter で表示用の `ChannelMapEntry` に正規化する
* `ChannelMapEntry.sourceType` は `own_team` / `participant_team` / `observed_wifi` とし、色、凡例、フィルタ、ツールチップの判定に使う
* 他チームの連絡先など公開対象外の情報は、`ChannelMapEntry` に含めない
* IssueReport は API 層で自チーム・運営向けの詳細 DTO と、他チーム向けの公開要約 DTO を分ける
* 他チーム向けの公開要約 DTO には `visibility = team_public` の報告だけを含め、報告者名、自由記述、添付、詳細位置などを含めない
* 新しい永続テーブルは追加せず、既存の `WifiConfig` と `ObservedWifi` から派生させる
...
* チャンネル番号と帯域の妥当性チェック
* チャンネル幅から占有範囲を計算する関数
* チャンネルマップ表示用の `ChannelMapEntry` 型と sourceType enum
* 同期 payload 型
* IssueReport の公開範囲 enum と公開要約 schema

最初にテストを厚くする対象:

* 2.4GHz / 5GHz / 6GHz のチャンネル妥当性
* チャンネル幅から表示範囲を出す計算
* `ChannelMapEntry.sourceType` ごとの表示ラベル、フィルタ、公開情報制御
* WiFi 構成最大 3 件ルール
* IssueReport の簡易モード必須項目
* IssueReport の公開範囲ごとのレスポンス制御
...
* セッション、認証、サーバ側同期受付状態は DB または署名済み Cookie に置く
* オフライン中の `pending` / `failed` などの同期キュー状態は端末内 DB を正とし、サーバ正本の IssueReport などへ混ぜない
