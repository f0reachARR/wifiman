...
ALTER TABLE "sync_records" ADD CONSTRAINT "sync_records_status_check" CHECK ("sync_records"."status" IN ('pending', 'processing', 'failed', 'done'));--> statement-breakpoint
CREATE UNIQUE INDEX "team_accesses_single_active_per_team" ON "team_accesses" USING btree ("team_id") WHERE "team_accesses"."revoked_at" IS NULL;