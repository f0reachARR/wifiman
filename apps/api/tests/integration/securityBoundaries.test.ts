...
process.env.DATABASE_URL ??=
  "postgres://postgres:postgres@localhost:5432/wifiman_test";
...
const { mockDb, state } = vi.hoisted(() => {
  ...
  const db = {
    query: {
      issueReports: {
        findFirst: vi.fn(
          async () => mockState.findFirstQueue.issueReports.shift() ?? null,
        ),
      },
      wifiConfigs: {
        findFirst: vi.fn(
          async () => mockState.findFirstQueue.wifiConfigs.shift() ?? null,
        ),
      },
      teams: {
        findFirst: vi.fn(
          async () => mockState.findFirstQueue.teams.shift() ?? null,
        ),
      },
      tournaments: {
        findFirst: vi.fn(
          async () => mockState.findFirstQueue.tournaments.shift() ?? null,
        ),
      },
      deviceSpecs: {
        findFirst: vi.fn(
          async () => mockState.findFirstQueue.deviceSpecs.shift() ?? null,
        ),
      },
    },
    select: vi.fn(() => ({
...
describe("security boundaries integration", () => {
  it("viewer は他チームの公開情報を閲覧できるが機密はマスクされる", async () => {
    ...
    const deviceBody = (await deviceRes.json()) as Array<
  ...
  it("通常ログイン user は channel-map / issueReports 一覧・summary を閲覧できない", async () => {
    ...
    const channelMapRes = await app.request(
      `/api/tournaments/${ids.tournamentA}/channel-map`,
      {
        method: "GET",
        headers: { "x-test-auth": auth },
      },
  ...
  it("別大会トークンは参加者限定の閲覧系APIをクロストーナメントで閲覧できない", async () => {
    const app = await createTestApp();
    const crossTournamentAuth = JSON.stringify(
      participantAuth(ids.teamB, "viewer"),
    );
    ...
    const issueListRes = await app.request(
      `/api/tournaments/${ids.tournamentA}/issue-reports`,
      {
        method: "GET",
        headers: { "x-test-auth": crossTournamentAuth },
    ...
    const wifiListRes = await app.request(
      `/api/teams/${ids.teamA}/wifi-configs`,
      {
        method: "GET",
        headers: { "x-test-auth": crossTournamentAuth },
      },
    ...
    const deviceListRes = await app.request(
      `/api/teams/${ids.teamA}/device-specs`,
      {
        method: "GET",
        headers: { "x-test-auth": crossTournamentAuth },
      },
  ...
  it("observedWifis 公開APIでは notes を返さない", async () => {
    ...
    const res = await app.request(
      `/api/tournaments/${ids.tournamentA}/observed-wifis`,
      {
        method: "GET",
      },
    );
