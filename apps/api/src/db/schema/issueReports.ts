...
export const issueReports = pgTable(
  "issue_reports",
  {
    ...
    wifiConfigId: text("wifi_config_id").references(() => wifiConfigs.id, {
      onDelete: "set null",
    }),
    ...
    distanceCategory: text("distance_category").$type<
      "near" | "mid" | "far" | "obstacle"
    >(),
    estimatedDistanceMeters: real("estimated_distance_meters"),
    locationLabel: text("location_label"),
    reproducibility: text("reproducibility").$type<
  ...
  (t) => [
    ...
    check(
      "issue_reports_band_check",
      sql`${t.band} IN ('2.4GHz', '5GHz', '6GHz')`,
    ),
    check(
