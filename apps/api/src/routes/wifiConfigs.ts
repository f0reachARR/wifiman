...
const wifiConfigResponseSchema = WifiConfigSchema;
const publicWifiConfigResponseSchema = wifiConfigResponseSchema.omit({
  pingTargetIp: true,
  notes: true,
});
const listWifiConfigResponseSchema = z.array(
  z.union([wifiConfigResponseSchema, publicWifiConfigResponseSchema]),
);
const deleteResponseSchema = z.object({ message: z.string() });
type AppMiddleware = MiddlewareHandler<{ Variables: ContextVariableMap }>;

function toPublicWifiConfig(row: Record<string, unknown>) {
...
async function assertDeviceBelongsToTeam(
  ...
  if (!deviceId) return;
  const device = await db.query.deviceSpecs.findFirst({
    where: eq(deviceSpecs.id, deviceId),
  });
  if (!device) {
...
app.openapi(listWifiConfigs, async (c) => {
  ...
  if (authCtx?.userRole === 'operator' || authCtx?.teamId === teamId) {
    return c.json(z.array(wifiConfigResponseSchema).parse(rows), 200);
  }
  return c.json(
    listWifiConfigResponseSchema.parse(rows.map((row) => toPublicWifiConfig(row))),
    200,
  );
});
...
const createWifiConfig = createRoute({
  ...
  request: {
    ...
    body: {
      content: {
        'application/json': {
          schema: CreateWifiConfigSchema.omit({ teamId: true }),
        },
      },
      required: true,
    },
...
app.openapi(createWifiConfig, async (c) => {
  ...
  return c.json(wifiConfigResponseSchema.parse(row), 201);
});
...
const patchWifiConfig = createRoute({
  ...
  request: {
    ...
    body: {
      content: { 'application/json': { schema: UpdateWifiConfigSchema } },
      required: true,
    },
  },
  responses: {
...
app.openapi(patchWifiConfig, async (c) => {
  ...
  const existing = await db.query.wifiConfigs.findFirst({
    where: eq(wifiConfigs.id, id),
  });
  ...
  return c.json(wifiConfigResponseSchema.parse(row), 200);
});

// DELETE /api/wifi-configs/:id - WiFi 構成無効化 (team_editor)
...
app.openapi(deleteWifiConfig, async (c) => {
  ...
  const [row] = await db
    ...
    .returning();
  if (!row) throw notFound('WiFi 構成が見つかりません');
  return c.json({ message: '無効化しました' }, 200);
});
