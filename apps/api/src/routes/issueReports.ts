...
function toPublicIssueReportSummary(
  row: z.infer<typeof issueReportResponseSchema>,
) {
  const {
...
app.openapi(listIssueReports, async (c) => {
  ...
  return c.json(
    visibleRows.map((row) =>
      canViewIssueReportDetail(authCtx, row)
        ? row
        : toPublicIssueReportSummary(row),
    ),
    200,
  );
});
...
app.openapi(summaryIssueReports, async (c) => {
  ...
  const bySeverity = { low: 0, medium: 0, high: 0, critical: 0 } as Record<
    string,
    number
  >;
  const byBand = { "2.4GHz": 0, "5GHz": 0, "6GHz": 0 } as Record<
    string,
    number
  >;
  const channelCounts = new Map<
    string,
    { band: string; channel: number; count: number }
  >();
...
app.openapi(createIssueReport, async (c) => {
  ...
  if (body.wifiConfigId) {
    ...
    finalBody = {
      ...
      channelWidthMHz:
        finalBody.channelWidthMHz ?? config.channelWidthMHz ?? undefined,
    };
  ...
  validateIssueReportChannel(
    finalBody.band,
    finalBody.channel,
    finalBody.channelWidthMHz,
  );
...
app.openapi(updateIssueReport, async (c) => {
  ...
  if (
    body.wifiConfigId !== undefined &&
    body.wifiConfigId !== existing.wifiConfigId
  ) {
